...
Esurfce = flip(Esurfce);
set(Earth,'Facecolor','texturemap','Edgecolor','none','Cdata',Esurfce);

xlabel('X [km]'); ylabel('Y [km]'); zlabel('Z [km]');

title('Evolution of the orbit','Color',[1 1 1],'FontSize',18);
...
for i = 1 : Rows
    ...
    for j = 1 : length(Orbit_th)
        [R,V] = orbitalToCar(a,e,inc,Omega,omega,Orbit_th(j),mu);
        Orbit(:,j) = R;
    end
    ...
    for j = 1 : length(Orbit_path_th)
        [R,V] = orbitalToCar(a,e,inc,Omega,omega,Orbit_path_th(j),mu);
        Orbit_path(:,j) = R;
    end
